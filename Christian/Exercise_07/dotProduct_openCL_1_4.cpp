...
""
"__kernel void dotProductFirstStep(__global double *x,\n"
"                      __global double *y,\n"
"                       __global double *dot, \n"
"                      unsigned int N\n)"
"{\n"
...
int main()
{
  ...
  for (int j = 0; j < M; j++) {
    ...
    char* rrrr[source_len+1];
    char* f = " ";
    strcpy(rrrr, my_opencl_program);
    strcat(rrrr, f;)
    ...
    //
    cl_kernel my_kernel = clCreateKernel(prog, "dotProductFirstStep", &err); OPENCL_ERR_CHECK(err);
  }
  ...
  std::vector<ScalarType> dot(vector_size, 0);
  ...
  std::vector<double> timings;
    for(int reps=0; reps < 10; ++reps) {
        ...
        cl_mem ocl_dot = clCreateBuffer(my_context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, vector_size * sizeof(ScalarType), &(dot[0]), &err); OPENCL_ERR_CHECK(err);
        ...
        err = clSetKernelArg(my_kernel, 2, sizeof(cl_mem),  (void*)&ocl_dot); OPENCL_ERR_CHECK(err);
        ...
        err = clEnqueueReadBuffer(my_queue, ocl_dot, CL_TRUE, 0, sizeof(ScalarType) * dot.size(), &(dot[0]), 0, NULL, NULL); OPENCL_ERR_CHECK(err);


        // sum the elements of dot
        result = 0;
        result = std::accumulate(dot.begin(), dot.end(), result);
  ...
  std::cout << "Calculation of Dot Product took " << time_elapsed << std::endl << std::endl;
